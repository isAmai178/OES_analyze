...
class OESAnalyzerGUI(QMainWindow):
    ...
    def _analyze_data(self):
        ...
        try:
            selected_folder = self.folder_selection_combo.currentText()  # 獲取選擇的資料夾
            if not selected_folder:
                raise ValueError("請選擇要分析的資料夾")

            self.controller.load_and_process_data(selected_folder, self.base_name, self.start_index, self.end_index)
